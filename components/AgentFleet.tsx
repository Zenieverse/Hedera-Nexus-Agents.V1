...
const AgentFleet: React.FC<AgentFleetProps> = ({ agents, selectedAgentId, onSelectAgent, onStopAgent }) => {
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-center">
                                            <div className="flex items-center space-x-2">
                                                ...
                                                <span className={`text-[10px] px-1 rounded border ${getLevelBadge(agent.level)}`}>
                                                    LVL {agent.level}
                                                </span>
                                            </div>
                                            <p className={`text-xs font-medium ${getBalanceColor(agent.hbarBalance)}`}>Ħ{agent.hbarBalance.toFixed(2)}</p>
                                        </div>
                                        <p className="text-xs text-gray-400 truncate mt-0.5">{agent.taskDescription}</p>
                                    </div>
